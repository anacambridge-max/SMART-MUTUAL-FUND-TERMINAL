...
async function searchFund(f:FundConfig):Promise<SearchRow|null>{
  try{const q=encodeURIComponent(`${f.schemeSearch} direct growth`);const r=await fetch(`${BASE}/api/v1/search?q=${q}`,{cache:"no-store"});if(!r.ok)return null;const j=await r.json() as SearchResponse;const rows=(j.data||[]).filter(x=>Number.isFinite(Number(x.scheme_code)));const wanted=f.schemeSearch.toLowerCase();return rows.find(x=>x.scheme_name.toLowerCase().includes(wanted)&&x.scheme_name.toLowerCase().includes("direct"))||rows.find(x=>x.scheme_name.toLowerCase().includes("direct"))||rows[0]||null;}catch{return null}
}

async function history(code:number):Promise<Point[]>{
  try{const r=await fetch(`${BASE}/api/v1/schemes/${code}/nav/history?limit=260`,{cache:"no-store"});if(!r.ok)return [];const j=await r.json() as HistoryResponse;return (j.data||[]).map(x=>({at:parseDate(x.date),value:Number(x.nav)})).filter((x):x is Point=>!!x.at&&Number.isFinite(x.value)).sort((a,b)=>a.at.getTime()-b.at.getTime());}catch{return []}
}
...
function compute(f:FundConfig,row:SearchRow,p:Point[],indices:IndexDashboardRow[],settings:SettingsPayload):FundComputed{
  const latest=p.at(-1)?.value??row.nav;const m=metrics(p);const sector=weighted(f.sectorExposure,indices);const proxy=indexMove(indices,f.proxyIndex);
...
export async function buildLiveFundPayload(base:DashboardPayload):Promise<DashboardPayload>{
  ...
  const resolved=await Promise.all(settings.fundsConfig.map(async f=>{const row=await searchFund(f);const points=row?await history(row.scheme_code):[];const usable=row&&(points.length?points:[{at:new Date(),value:row.nav}]);return {fund:f,row,points:usable||[]};}));
  const funds=resolved.map(x=>x.row?compute(x.fund,x.row,x.points,indices,settings):fallback(x.fund));
  ...
  return {...base,funds,topFunds,avoidFunds,tacticalAllocation,sourceStatus:{...base.sourceStatus,amfi:liveCount?"fallback":"unavailable",note:`No database. NSE/index data is live; ${liveCount}/${funds.length} mutual funds resolved through mfdata.in.`}};
}
