...
type SearchRow = { scheme_code:number; scheme_name:string; nav:number; aum_cr?:number };
type SearchResponse = { status?:string; data?:SearchRow[] };
type HistoryPoint = { date:string; nav:number };
type HistoryResponse = { status?:string; data?:HistoryPoint[] };

type Point = { at:Date; value:number };

const BASE="https://mfdata.in";
const clamp=(v:number,a=0,b=100)=>Math.max(a,Math.min(b,v));
const round=(v:number,d=2)=>Math.round(v*10**d)/10**d;
const pct=(a:number|null,b:number|null)=>a==null||b==null||a===0?null:((b-a)/a)*100;
const parseDate=(s:string)=>{const [y,m,d]=s.split("-").map(Number);return y&&m&&d?new Date(Date.UTC(y,m-1,d)):null};
...
const weighted=(exposure:Record<string,number>,rows:IndexDashboardRow[])=>{let sum=0,w=0,count=0;for(const [name,weight] of Object.entries(exposure||{})){if(weight<=0)continue;const move=indexMove(rows,name);sum+=move*weight;w+=weight;count++;}return {move:w?sum/w:0,count};};

async function searchFund(f:FundConfig):Promise<SearchRow|null>{
  try{
    const q=encodeURIComponent(`${f.schemeSearch} direct growth`);
    const r=await fetch(`${BASE}/api/v1/search?q=${q}`,{cache:"no-store"});
    if(!r.ok)return null;
    const j=await r.json() as SearchResponse;
    const rows=(j.data||[]).filter(x=>Number.isFinite(Number(x.scheme_code)));
    const wanted=f.schemeSearch.toLowerCase();
    return rows.find(x=>x.scheme_name.toLowerCase().includes(wanted)&&x.scheme_name.toLowerCase().includes("direct"))||rows.find(x=>x.scheme_name.toLowerCase().includes("direct"))||rows[0]||null;
  }catch{return null}
}

async function history(code:number):Promise<Point[]>{
  try{
    const r=await fetch(`${BASE}/api/v1/schemes/${code}/nav/history?limit=260`,{cache:"no-store"});
    if(!r.ok)return [];
    const j=await r.json() as HistoryResponse;
    return (j.data||[]).map(x=>({at:parseDate(x.date),value:Number(x.nav)})).filter((x):x is Point=>!!x.at&&Number.isFinite(x.value)).sort((a,b)=>a.at.getTime()-b.at.getTime());
  }catch{return []}
}
...
function metrics(p:Point[]){
  ...
  return {
    ...
    momentum10d:pct(before(p,10)?.value??null,latest),momentum20d:pct(before(p,20)?.value??null,latest),momentum50d:pct(before(p,50)?.value??null,latest),relativeStrengthVsNifty50d:null as number|null
  };
}
...
function compute(f:FundConfig,row:SearchRow,p:Point[],indices:IndexDashboardRow[],settings:SettingsPayload):FundComputed{
  const latest=p.at(-1)?.value??row.nav;
  const m=metrics(p);const sector=weighted(f.sectorExposure,indices);const proxy=indexMove(indices,f.proxyIndex);
...
export async function buildLiveFundPayload(base:DashboardPayload):Promise<DashboardPayload>{
  ...
  const resolved=await Promise.all(settings.fundsConfig.map(async f=>{const row=await searchFund(f);return {fund:f,row,points:row?await history(row.scheme_code):[]}}));
  const funds=resolved.map(x=>x.row&&x.points.length?compute(x.fund,x.row,x.points,indices,settings):fallback(x.fund));
  const topFunds=[...funds].sort((a,b)=>b.finalDailyScore-a.finalDailyScore).slice(0,5);const avoidFunds=funds.filter(f=>f.actionTag==="AVOID TODAY");
  ...
  const liveCount=funds.filter(f=>f.latestNav!=null).length;
  return {...base,funds,topFunds,avoidFunds,tacticalAllocation,sourceStatus:{...base.sourceStatus,amfi:liveCount?"fallback":"unavailable",note:`No database. NSE/index data is live; ${liveCount}/${funds.length} mutual funds resolved through mfdata.in with NAV history.`}};
}
